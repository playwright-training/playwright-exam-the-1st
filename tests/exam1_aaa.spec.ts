...
test.describe("①AAA設計課題：レポートページ", () => {
  ...
  test("担当者「田中」で絞り込むとテーブルが2件になる", async ({ page }) => {
    ...
    const reportPage = new ReportPage(page);
    await page.getByTestId("report-period").selectOption("全期間");
    await reportPage.filterByAssignee("田中");
    await expect(reportPage.getReportRowCount()).resolves.toBe(2);
  });

  // ────────────────────────────────────────────
  // 課題1-2: 集計カードの件数
  // ────────────────────────────────────────────
  test("集計カードの「総タスク数」がテーブルの行数と一致する", async ({
    page,
  }) => {
    ...
    await expect(reportPage.getReportRowCount()).resolves.toBe(stats[0]);
  });

  // ────────────────────────────────────────────
  // 課題1-3: エクスポートボタン
  // ────────────────────────────────────────────
  test("エクスポートボタンをクリックするとトーストが表示される", async ({
    page,
  }) => {
